...
function wrapText(text,font,size,maxWidth){const words=clean(text).split(/\s+/);const lines=[];let line='';for(const word of words){const candidate=line?line+' '+word:word;if(font.widthOfTextAtSize(candidate,size)<=maxWidth){line=candidate;continue;}if(line)lines.push(line);if(font.widthOfTextAtSize(word,size)<=maxWidth){line=word;continue;}let part='';for(const ch of word){const test=part+ch;if(font.widthOfTextAtSize(test,size)>maxWidth&&part){lines.push(part);part=ch;}else part=test;}line=part;}if(line)lines.push(line);return lines.length?lines:[''];}
