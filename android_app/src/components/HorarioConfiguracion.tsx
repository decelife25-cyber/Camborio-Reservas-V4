...
export default function HorarioConfiguracion({onBack,darkMode,onToggleTheme}:Props){
 ...
 const contenido=<section className="cr-horarios-fullscreen"><div className="cr-horarios-fullscreen__surface"><header className="cr-horarios-fullscreen__header"><h2>⚙ CONFIGURACIÓN / HORARIOS</h2><div className="cr-config-modal__header-actions"><button className="cr-horarios-fullscreen__save" type="button" disabled={!dirty||saving} onClick={()=>void save()}>{saving?'GUARDANDO':'GUARDAR'}</button><button className="cr-config-modal__theme" type="button" onClick={onToggleTheme} aria-label="Cambiar modo día/noche">{darkMode?<Sun size={22}/>:<Moon size={22}/>}</button><button className="cr-config-modal__close" type="button" onClick={onBack}>×</button></div></header><section className="cr-config-horarios">{loading?<div className="cr-horario-vacio">CARGANDO HORARIOS...</div>:<>
