...
export default function HorarioConfiguracion({onBack,darkMode,onToggleTheme}:Props){
 ...
 const load=async(ok?:string)=>{
  ...
  (q.data||[]).forEach((raw:any)=>{
   ...
   if(s!=='COMIDA'&&s!=='CENA')return;
   const row:Horario={DiaSemana:d,Servicio:s,Hora:h,MargenHoras:raw.MargenHoras==null?null:Number(raw.MargenHoras),Activo:Boolean(raw.Activo)};
   const key=d+'|'+s+'|'+h,prev=dataMap.get(key);
   // If legacy data has casing duplicates, prefer the canonical day spelling.
   if(!prev || String(raw.DiaSemana)===d)dataMap.set(key,row);
  });
 ...
 const cambiosPendientes=()=>{if(deleted.length)return true;const key=(r:Horario)=>r.DiaSemana+'|'+r.Servicio+'|'+r.Hora;const a=new Map(initial.map(r=>[key(r),r]));const b=new Map(rows.map(r=>[key(r),r]));if(a.size!==b.size)return true;for(const [k,o] of a){const n=b.get(k);if(!n||!!o.Activo!==!!n.Activo||Number(o.MargenHoras)!==Number(n.MargenHoras))return true}for(const s of ['COMIDA','CENA'] as Servicio[]){const vals=Array.from(new Set(initial.filter(r=>r.Servicio===s&&r.MargenHoras!=null).map(r=>Number(r.MargenHoras))));const im=vals.length===1?vals[0]:(vals.length?null:2);if(Number(margin[s])!==Number(im))return true}return false};
 const dirty=cambiosPendientes();

 const save=async()=>{if(Object.keys(bad).length){setMessage('No se sobrescribirán márgenes distintos sin una regla de migración confirmada.');setError(true);return}if((['COMIDA','CENA'] as Servicio[]).some(s=>margin[s]==null||margin[s]!<0||margin[s]!>999.99)){setMessage('Indica una antelación mínima válida para COMIDA y CENA.');setError(true);return}const now=rows.map(r=>({...r,MargenHoras:margin[r.Servicio]})),old=new Map(initial.map(r=>[r.DiaSemana+'|'+r.Servicio+'|'+r.Hora,r])),changes=now.filter(r=>{const o=old.get(r.DiaSemana+'|'+r.Servicio+'|'+r.Hora);return !o||!!o.Activo!==!!r.Activo||Number(o.MargenHoras)!==Number(r.MargenHoras)});setSaving(true);setMessage('');setError(false);try{for(const r of changes){const q=await supabase.from('Horarios').select('DiaSemana').ilike('DiaSemana',r.DiaSemana).eq('Servicio',r.Servicio).eq('Hora',r.Hora+':00').limit(1);if(q.error)throw q.error;if(q.data?.length){const u=await supabase.from('Horarios').update({Activo:r.Activo,MargenHoras:r.MargenHoras}).ilike('DiaSemana',r.DiaSemana).eq('Servicio',r.Servicio).eq('Hora',r.Hora+':00').select('DiaSemana');if(u.error)throw u.error;if(!u.data?.length)throw new Error('No se actualizó el horario '+r.DiaSemana+' '+r.Servicio+' '+r.Hora)}else{const i=await supabase.from('Horarios').insert({DiaSemana:r.DiaSemana,Servicio:r.Servicio,Hora:r.Hora+':00',MargenHoras:r.MargenHoras,Activo:r.Activo}).select('DiaSemana');if(i.error)throw i.error;if(!i.data?.length)throw new Error('No se insertó el horario '+r.DiaSemana+' '+r.Servicio+' '+r.Hora)}}for(const r of deleted){const d=await supabase.from('Horarios').delete().eq('Servicio',r.Servicio).eq('Hora',r.Hora+':00');if(d.error)throw d.error}await load('Horarios guardados correctamente.')}catch(e){console.error('[Horarios] save',e);setMessage('No se pudieron guardar los horarios.');setError(true)}finally{setSaving(false)}};
 ...
 const contenido=<section className="cr-horarios-fullscreen"><div className="cr-horarios-fullscreen__surface"><header className="cr-horarios-fullscreen__header"><h2>⚙ CONFIGURACIÓN / HORARIOS</h2><div className="cr-config-modal__header-actions"><button className="cr-horarios-fullscreen__save" type="button" disabled={!dirty||saving} onClick={()=>void save()}>{saving?'GUARDANDO':'GUARDAR'}</button><button className="cr-config-modal__theme" type="button" onClick={onToggleTheme} aria-label="Cambiar modo día/noche">{darkMode?<Sun size={22}/>:<Moon size={22}/>}</button><button className="cr-config-modal__close" type="button" onClick={()=>dirty?setShowExitConfirm(true):onBack()} aria-label="Cerrar">×</button></div></header><section className="cr-config-horarios">{loading?<div className="cr-horario-vacio">CARGANDO HORARIOS...</div>:<>
  ...
  {message&&<div className="cr-config-mensaje" data-tipo={error?'error':'ok'}>{message}</div>}
 </>}{pending&&<section className="cr-horario-confirmar"><p>¿Eliminar esta hora de todos los días de este servicio?</p><div><button className="cr-button cr-button--dark" type="button" onClick={()=>setPending(null)}>CANCELAR</button><button className="cr-button cr-button--danger" type="button" onClick={remove}>ELIMINAR</button></div></section>}</section>{showExitConfirm&&<section className="cr-horario-salir-confirmar" role="dialog" aria-modal="true" aria-labelledby="cr-horario-salir-titulo"><div className="cr-horario-salir-confirmar__box"><h3 id="cr-horario-salir-titulo">CAMBIOS SIN GUARDAR</h3><p>Has realizado cambios en los horarios. ¿Quieres salir sin guardar?</p><div className="cr-horario-salir-confirmar__acciones"><button type="button" className="cr-button cr-button--dark" onClick={()=>setShowExitConfirm(false)}>SEGUIR EDITANDO</button><button type="button" className="cr-button cr-button--danger" onClick={onBack}>SALIR SIN GUARDAR</button></div></div></section>}</div></section>;
