...
export default function SearchReservationCard({reserva:initial,index,total,onNavigate,onUpdated}:{reserva:SearchReservation;index:number;total:number;onNavigate:(d:number)=>void;onUpdated:(r:SearchReservation)=>void}){
 ...
 return <div className="cr-busqueda-ficha-wrap">
  ...
  {stateOpen&&<div className="v2-edit-overlay" onClick={()=>setStateOpen(false)}><div className="v2-edit-modal" onClick={e=>e.stopPropagation()}><h3>CAMBIAR ESTADO</h3><div className={"v2-state-current status-modal-" + r.Estado.toLowerCase().replaceAll("_","-")}>{stateLabel(r.Estado)}</div><div className="v2-edit-actions">{stateActions.map(([s,label])=><button key={s} type="button" className={'v2-state-action v2-state-action--'+(s==='CONFIRMADA'?'confirmar':s==='SENTADA'?'sentar':s==='FINALIZADA'?'finalizar':s==='NO_PRESENTADO'?'no-presentado':'cancelar')} onClick={()=>changeState(s)} disabled={saving}>{saving?'...':label}</button>)}</div><button className="v2-edit-cancel-full" type="button" onClick={()=>setStateOpen(false)}>CERRAR SIN CAMBIOS</button></div></div>}
 </div>;
}
