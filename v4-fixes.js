...
function version(){if(!$('deployment-version')){const v=document.createElement('div');v.id='deployment-version';v.className='deployment-version';v.textContent='Camborio Reservas V4 · V4.0.0.18';document.body.appendChild(v)}}
function finish(){if($('finish-button'))$('finish-button').onclick=function(){if($('screen-thanks')){const card=$('screen-thanks .thanks-card');if(card&&!$('thanks-menu')){const menu=document.createElement('button');menu.id='thanks-menu';menu.type='button';menu.className='info';menu.textContent='VER CARTA DIGITAL';menu.onclick=function(){window.location.href='https://decelife.com/carta-digital/'};card.appendChild(menu)}showScreen('screen-thanks')}}}
